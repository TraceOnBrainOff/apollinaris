...
function rCS:loadSpeciesData() --loads all the predefined configs for species into memory and creates a virtual pie menu for selection
	...
	for i, species in ipairs(speciesList) do
		local species_config = root.assetJson(string.format("/apollo_skills/rCS/%s.json", species), {})
	...
	self.speciesSelectMenu = VirtualPie:new(root.assetJson("/apollo_skills/rCS/speciesSelectMenu.config", {}), {newRing})
end
